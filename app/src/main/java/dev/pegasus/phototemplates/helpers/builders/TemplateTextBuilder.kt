...
class TemplateTextBuilder(
    ...
    fun setStickers(model: TemplateModel? = null, transformedWidth: Float, transformedHeight: Float) {
        model?.let { template ->
            if (template.stickers?.isNotEmpty() == true){
                template.stickers?.forEachIndexed { _, sticker ->
                    binding?.stvHome?.addPredefinedSticker(sticker, template.width, template.height, transformedWidth, transformedHeight)
